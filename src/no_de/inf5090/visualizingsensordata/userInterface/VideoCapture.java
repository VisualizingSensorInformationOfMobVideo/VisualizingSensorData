...
public class VideoCapture extends Activity {
	...
	private void initCameraStuff() {
		...
		// TODO: this is not necessary, as the preview class will inject into
	 ...
	 * 
	 * public void setAppDir(File appDir) { VideoCapture.appDir = appDir; }
	 */

	/**
	 * Toggles the visibility of the sensor warnings when the toggle button is
	 * pressed.
	 * 
	 * @param view
	 *            The toggle button view.
	 */
	...
	public void locationUpdate(LocationSensorObserver sensor) {
		...
		// TODO: our project really don't need a location, so we don't really
		// need this
		mRecordButton.setEnabled(true);
	}
	...
	private void stopSendingSnapshot() {
		sendingSnapshot = false;
	}

	private void startSendingSnapshot() {
	...
	PictureCallback jpegCallback = new PictureCallback() {
		public void onPictureTaken(byte[] data, Camera camera) {
			...
			mCameraHelper.getCamera().startPreview(); // to avoid preview
														// freezing after taking
														// a pic

			new SnapshotWriter().execute(data);
			Log.d("snap", "onPictureTaken - jpeg");
			/*
			 * snapshotTransmission.send_snapshot(); if (sendingSnapshot) {
			...
			if (snapshotCounter <= numOfSnapshots) {
				Thread thread = new Thread() {
					...
					public void run() {
						try {
							sleep(delay);
							mCameraHelper.getCamera().takePicture(null, null,
									jpegCallback);
	...
	public void stopPersistingSensorData() {
		...
		mLocalStorageWriter.writeXml(appDir.getPath() + "/"
				+ correspondingFileName + "-sensor.xml");
	}
